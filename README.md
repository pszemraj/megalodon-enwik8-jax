...
A controlled JAX/Equinox comparison of [Megalodon](https://arxiv.org/abs/2404.08801) and a paper-scaled Llama 2 baseline on byte-level enwik8. Megalodon is provided by [`megalodon-jax==0.2.1`](https://github.com/pszemraj/megalodon-jax).
...
Megalodon has lower loss in all three primary pairs and all three twice-horizon pairs, including the secondary comparison at a common XLA-estimated training-compute budget. Llama is 1.62–1.63× faster in synchronized steady-state training at this short context. See [RESULTS.md](RESULTS.md) for the architecture derivation, LR pilot, every paired result, validation trajectories, compute/memory accounting, caveats, and interpretation.
...
Python 3.11 or newer is required. The project pins the modeling backend and numerical stack:
...
The separately declared twice-horizon extension uses the same locked peak rates with a 12-update warmup and fresh cosine schedule:
...
Inference accepts a completed run directory and validates its manifest, model type, payload name, and parameter count before generation:

```bash
XLA_PYTHON_CLIENT_PREALLOCATE=false python inference.py --run-dir runs/my_megalodon_run --prompt 'The ' --top-k 64 --top-p 0.95
```

Each run has one model payload: native `model.safetensors` for Megalodon or atomic `model.eqx` for Llama. Sampling uses the common `temperature`, `top_k`, and `top_p` controls. Llama inference is an eager qualitative-sampling path and rejects requests whose prompt plus continuation would exceed its precomputed RoPE capacity.
...
- The full-shape Megalodon preflight added 18,033 MB of live framebuffer use with JAX preallocation disabled; allow 20 GB of free VRAM for reproduction.
...
- Cold compilation is reported separately. Steady-state timing blocks on GPU completion, excludes the first 10 updates, and excludes validation and generation. Every validation point records tokens and, when XLA exposes compiler cost analysis, cumulative estimated training FLOPs.
...
```text
megalodon-enwik8-jax/
├── configs/                       # Audited model and training contracts
...
│   └── utils.py                   # Data, training, generation, and artifacts
...
The suite covers paper-scaled parameter counts, Llama initialization policy, full-versus-cached logits, partial Megalodon chunks, AdamW semantics, schedule endpoints, accumulation equivalence, fixed validation windows, run reuse, compute-matched interpolation, both artifact round-trips, paired aggregation, and CPU/GPU smoke paths.
